...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" fpi="http://schemas.opengis.net/sensorML/2.0/aggregate_process.sch" queryBinding="xslt2">
    ...
    <sch:pattern>
        ...
        <sch:rule context="sml:AggregateProcess">
            <sch:assert test="count(gml:identifier) = 1">
                There shall be one and only one "gml:identifier" for any AggregateProcess. (Req 46)
            </sch:assert>
        </sch:rule>
    </sch:pattern>
    <sch:pattern>
        <sch:title>Req 46</sch:title>
        <sch:rule context="sml:AggregateProcess/gml:identifier">
