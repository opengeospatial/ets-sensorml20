...
mvn install
cd target 
unzip ets-sensorml20-2.0.0-r1.zip -d $TE_BASE/scripts/
...
echo "  "
echo "TEAM Engine ($te_tag) and the ets-sensorml20-2.0.0-r1 have been installed."
echo "  "
echo " UNIX/OS users - You can run the test with the following commands: "
echo "   export TE_BASE=$TE_BASE"
echo "   $base/teamengine/te_install/bin/unix/test.sh -source=sensorml/2.0.0/ctl/SensorML.xml"
echo "  "
echo " Windows users - You can run the test with the following commands:"
echo "   set TE_BASE=$TE_BASE"
echo "   $base/teamengine/te_install/bin/windows/test.bat -source=sensorml/2.0.0/ctl/SensorML.xml"
echo "  "
